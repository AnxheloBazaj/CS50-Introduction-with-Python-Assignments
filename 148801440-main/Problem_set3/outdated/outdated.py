    ...
    while True: # infinite loop to keep prompting the user
        ...
        try: # try statement which will try the following code
            ...
            if '/' in date: # check if the input contains a slash(/)
                mm, dd , yyyy = map(int, date.split('/')) #Splitting the date at the slash into three parts mm, dd, and yyyy amd converting them to int

                if mm <= 12 and dd <=31:break # check if the month and day are within limits and break out of the loop
            ...
            if ',' in date: # check if the input contains a comma
                    ...
                    if mm in months: #check if the month is in the month dictionary
                        mm = months[mm] # asign the value of the month key to month variable
                        mm = int(mm) # convert the value to int
                        if dd <=31 and mm <= 12:break # check if the days and months are within limits then break out of the loop


        except ValueError:pass # handle the error
